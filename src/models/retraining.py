...
MODEL_AGE = Gauge(
    "model_age_seconds", "Age of the current model in seconds", ["model_name"]
)
...
class DataChangeHandler(FileSystemEventHandler):
    ...
    def _handle_data_change(self, config, file_path):
        ...
        self.retraining_manager.trigger_retraining(
            config.model_name, "data_change", f"Data file modified: {file_path}"
        )
...
class ModelRetrainingManager:
        ...
        for config in self.configs.values():
            if os.path.exists(os.path.dirname(config.data_path)):
                self.observer.schedule(
                    event_handler, os.path.dirname(config.data_path), recursive=False
                )
                logger.info(
                    f"Monitoring data directory: {os.path.dirname(config.data_path)}"
                )
    ...
    def _monitoring_loop(self):
        ...
        while self.is_running:
            try:
                ...
                min_interval = min(
                    config.check_interval for config in self.configs.values()
                )
    ...
    def _check_time_based_triggers(self, config: RetrainingConfig):
        ...
        try:
            ...
            MODEL_AGE.labels(model_name=config.model_name).set(
                model_age.total_seconds()
            )
            ...
            if model_age > max_age:
                logger.info(
                    f"Model {config.model_name} is {model_age} old, triggering retraining"
                )
            ...
            logger.error(
                f"Error checking time-based triggers for {config.model_name}: {e}"
            )
    ...
    def _check_performance_triggers(self, config: RetrainingConfig):
        ...
        try:
            ...
            if degradation > config.performance_threshold:
                logger.info(
                    f"Performance degradation detected for {config.model_name}: {degradation:.3f}"
                )
                self.trigger_retraining(
                    config.model_name,
                    "performance",
            ...
            logger.error(
                f"Error checking performance triggers for {config.model_name}: {e}"
            )

    def trigger_retraining(
        self, model_name: str, trigger_type: str, description: str = ""
    ):
        ...
        RETRAINING_TRIGGERED.labels(
            model_name=model_name, trigger_type=trigger_type
        ).inc()

        # Start retraining in background thread
        retraining_thread = threading.Thread(
            target=self._perform_retraining, args=(config, trigger), daemon=True
        )
    ...
    def _perform_retraining(self, config: RetrainingConfig, trigger: RetrainingTrigger):
        ...
        try:
            ...
            if model_name == "iris":
                ...
                results = train_housing_model(
                    config.data_path, f"{model_name}_retraining"
                )
            else:
            ...
            # Update metrics
            RETRAINING_DURATION.labels(model_name=model_name, status="success").observe(
                duration
            )
            ...
            logger.info(
                f"Retraining completed successfully for {model_name} in {duration:.2f}s"
            )
            ...
            # Update metrics
            RETRAINING_DURATION.labels(model_name=model_name, status="failure").observe(
                duration
            )
            RETRAINING_FAILURE.labels(
    ...
    def _record_retraining(
        self, trigger: RetrainingTrigger, status: str, duration: float, **kwargs
    ):
    ...
    def get_retraining_status(self, model_name: str = None) -> Dict:
        ...
        status = {"is_running": self.is_running, "models": {}}
    ...
    def get_retraining_history(
        self, model_name: str = None, limit: int = 50
    ) -> List[Dict]:

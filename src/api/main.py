...
PREDICTION_COUNT = Counter(
    "ml_predictions_total", "Total ML Predictions", ["model_name", "status"]
)

PREDICTION_LATENCY = Histogram(
    "ml_prediction_duration_seconds", "ML Prediction Latency (seconds)", ["model_name"]
)

PREDICTION_CONFIDENCE = Histogram(
    "ml_prediction_confidence", "ML Prediction Confidence Scores", ["model_name"]
)

# Validation Metrics
VALIDATION_ERROR_COUNT = Counter(
    "validation_errors_total", "Total Validation Errors", ["model_name", "error_type"]
)

# Model Health Metrics
MODEL_LOAD_STATUS = Gauge(
    "model_load_status", "Model Load Status (1=loaded, 0=not_loaded)", ["model_name"]
)

MODEL_LAST_UPDATE = Gauge(
    "model_last_update_timestamp", "Last Model Update Timestamp", ["model_name"]
)
...
HOUSING_PRICE_PREDICTIONS = Summary(
    "housing_price_predictions", "Housing Price Predictions Statistics", ["status"]
)

IRIS_CLASS_PREDICTIONS = Counter(
    "iris_class_predictions_total", "Iris Class Predictions", ["predicted_class"]
)
...
async def startup_event():
    ...
    prediction_logger.log_model_status(
        "application", "startup", "YugenAI API application starting"
    )
...
async def shutdown_event():
    ...
    prediction_logger.log_model_status(
        "application", "shutdown", "YugenAI API application shutting down"
    )
    ...
    try:
        ...
        if response.status_code == 422:
            VALIDATION_ERROR_COUNT.labels(
                model_name="unknown", error_type="validation"
            ).inc()
...
prediction_logger.log_model_status(
    "iris", "loading", "Starting Iris model loading process"
)
...
try:
    ...
    if not os.path.exists(iris_model_path):
        ...
        prediction_logger.log_model_status(
            "iris", "loaded", f"Model type: {type(iris_model)}"
        )
        ...
        if not hasattr(iris_model, "predict"):
            ...
            prediction_logger.log_model_status(
                "iris", "ready", "Model loaded and ready for predictions"
            )
except Exception as e:
...
logger.info(f"MLFLOW_TRACKING_URI: {MLFLOW_TRACKING_URI}")
prediction_logger.log_model_status(
    "housing", "loading", "Starting Housing model loading process"
)
...
try:
    ...
    if (
        ...
        prediction_logger.log_model_status(
            "housing", "loaded", "Model and scaler loaded successfully"
        )
    else:
        ...
        feature_scaler = None
        logger.warning(
            "Warning: Loaded model does not include a scaler. Using unscaled features."
        )
        prediction_logger.log_model_status(
            "housing", "loaded", "Model loaded without scaler"
        )
    ...
    prediction_logger.log_model_status(
        "housing", "ready", "Model loaded and ready for predictions"
    )
        ...
        if log_type in ["prediction", "all"]:
            ...
            if prediction_log_file.exists():
                with open(prediction_log_file, "r") as f:
                    ...
                    log_content.extend(
                        [f"[PREDICTION] {line.strip()}" for line in pred_lines]
                    )
...
def predict_housing(features: HousingFeatures, request: Request):
    ...
    if housing_model is None:
        ...
        prediction_logger.log_response(
            request_id, "housing", None, processing_time, False, error_msg
        )
    ...
    try:
        ...
        for col in input_data:
            if col == "ocean_proximity":
                ...
                if ocean_col and ocean_col in input_df.columns:
                    ...
                    logger.warning(
                        f"Request {request_id}: Could not map ocean_proximity value '{input_data[col]}' to a valid feature column"
                    )
        ...
        if missing_cols:
            logger.warning(
                f"Request {request_id}: Missing expected columns: {missing_cols}"
            )
            for col in missing_cols:
                input_df[col] = 0

        logger.info(
            f"Request {request_id}: Processed input features: {input_df.iloc[0].to_dict()}"
        )
        ...
        if "feature_scaler" in globals() and feature_scaler is not None:
            ...
            input_df[numerical_cols] = feature_scaler.transform(
                input_df[numerical_cols]
            )
            logger.info(
                f"Request {request_id}: Applied feature scaling to numerical columns"
            )
        ...
        if (
            hasattr(housing_model, "target_transform_")
            and housing_model.target_transform_ == "log"
        ):
            prediction = np.expm1(prediction)  # Inverse of log1p
            logger.info(
                f"Request {request_id}: Transformed prediction (expm1): {prediction}"
            )
        ...
        confidence_score = (
            0.85  # Placeholder - could be calculated from model uncertainty
        )
        ...
        prediction_logger.log_response(
            request_id, "housing", float(prediction), processing_time, True
        )
        logger.info(
            f"Request {request_id}: Housing prediction completed successfully in {processing_time:.4f}s"
        )

        return HousingPredictionResponse(
            predicted_price=float(prediction), confidence_score=confidence_score
        )
...
def predict_iris(features: IrisFeatures, request: Request):
    ...
    if iris_model is None:
        ...
        processing_time = time.time() - start_time
        prediction_logger.log_response(
            request_id, "iris", None, processing_time, False, error_msg
        )
    ...
    try:
        ...
        # Log successful response
        prediction_logger.log_response(
            request_id, "iris", predicted_class, processing_time, True
        )
        logger.info(
            f"Request {request_id}: Iris prediction completed successfully in {processing_time:.4f}s"
        )
...
async def start_retraining_system():
    ...
    if retraining_manager and retraining_manager.is_running:
        raise HTTPException(
            status_code=400, detail="Retraining system is already running"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to start retraining system: {str(e)}"
        )
...
async def stop_retraining_system():
    ...
    if not retraining_manager or not retraining_manager.is_running:
        raise HTTPException(status_code=400, detail="Retraining system is not running")
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to stop retraining system: {str(e)}"
        )
...
async def get_retraining_status():
    ...
    try:
        if retraining_manager:
            ...
            configs = create_default_configs()
            status = {"is_running": False, "models": {}}
...
async def get_retraining_history(model_name: str = None, limit: int = 20):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get retraining history: {str(e)}"
        )
...
async def trigger_retraining(model_name: str):
    ...
    try:
        retraining_manager.trigger_retraining(
            model_name, "manual", "Manual trigger via API"
        )

...
class TestPredictionLogger:
    ...
    def test_log_request(self, tmp_path):
        ...
        with patch("src.utils.logger.setup_prediction_logger") as mock_setup:
            ...
            pred_logger.log_request(
                model_type, request_id, features, client_ip, user_agent
            )
    ...
    def test_log_response(self, tmp_path):
        ...
        with patch("src.utils.logger.setup_prediction_logger") as mock_setup:
            ...
            pred_logger.log_response(
                request_id, model_type, prediction, processing_time, success, error
            )
...
class TestLoggingPerformance:
    ...
    def test_logging_performance(self, tmp_path):
        ...
        with patch("src.utils.logger.setup_prediction_logger") as mock_setup:
            ...
            for i in range(100):
                pred_logger.log_request(
                    "iris", f"req-{i}", {"test": i}, "127.0.0.1", "test"
                )
                pred_logger.log_response(
                    f"req-{i}", "iris", "Iris-setosa", 0.001, True, None
                )
